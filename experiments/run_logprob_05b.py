#!/usr/bin/env python3
"""
LOGPROB CONFIDENCE COMPARISON — 0.5B MATH (2nd condition)
Validates token > logprob finding on a different model scale.
"""
...
def main():
    device = "cuda:0"
    print(f"Logprob 0.5B MATH | {datetime.now()}", flush=True)
    ...
    print(f"Loading Qwen2.5-0.5B from {model_path}", flush=True)
    ...
    ckpt_file = ckpt_dir / "logprob_confidence_math_05b.json"
    if ckpt_file.exists():
        ...
        for i in range(start, N_SAMPLES):
            ...
            if (i+1) % 20 == 0 or (i+1) == N_SAMPLES:
                ...
                with open(ckpt_file, 'w') as f:
                    json.dump({"metadata": {"model": "Qwen2.5-0.5B", "ds": "MATH",
    ...
    print(f"\n{'='*60}")
    print(f"TOKEN vs LOGPROB (Qwen2.5-0.5B MATH@256)")
    print(f"{'='*60}")
  ...
  ──────────────────────────────────────────────────────────────
  Token-length        {auc_tok:.3f}   {cohens_d(tok_c, tok_w):+.2f}       {p_mw_tok:.2e}  {'★★★' if p_mw_tok < 1e-10 else ''}
  Mean logprob        {auc_lp:.3f}   {cohens_d(lp_c, lp_w):+.2f}       {p_mw_lp:.2e}  {'NS!' if p_mw_lp > 0.05 else ''}
  ...
  Point-biserial:
    Token: r={r_tok:.3f}, p={p_tok:.2e}
    Logprob: r={r_lp:.3f}, p={p_lp:.2e}
""")
